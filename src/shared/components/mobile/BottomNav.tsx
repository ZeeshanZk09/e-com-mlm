    ...
    >
      <div className='relative'>
        ...
        {badge !== undefined && badge > 0 && (
          <span className='absolute -top-2 -right-2 bg-destructive text-destructive-foreground text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center animate-pulse'>
            {badge > 99 ? '99+' : badge}
          </span>
        )}
      </div>
      <span className={cn('text-xs font-medium', active && 'font-semibold')}>{label}</span>
      {active && <div className='absolute -bottom-1 w-12 h-1 bg-primary rounded-full' />}
    </Link>
  );
}
