...
import type { TCartListItemDB } from '@/shared/types/product';
...
const ShoppingCart = ({ isVisible, quantity, handleOnClose }: TProps) => {
  ...
  return (
    ...
    >
      <div
        className='absolute inset-0 sm:bg-black/60 bg-black/40 cursor-pointer'
      ...
      >
        <div className='flex items-center justify-between py-3 border-b border-gray-300 mx-6'>
          ...
          <Button
            onClick={handleOnClose}
            className='p-2 size-11 border-white hover:border-gray-300'
          >
            <CloseIcon width={18} />
          </Button>
        </div>
