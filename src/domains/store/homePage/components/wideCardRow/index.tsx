import type { CategoryOffer } from '@/actions/landing-dashboard/profile';
    ...
    <div className='w-full mt-15 flex flex-col gap-4 md:flex-row '>
      {cards?.map((card, idx) => {
        ...
        const newestOffers = _offers
          .slice()
          .sort((a, b) => {
            const aStarts = a.offer && a.offer.startsAt ? new Date(a.offer.startsAt).getTime() : 0;
            const bStarts = b.offer && b.offer.startsAt ? new Date(b.offer.startsAt).getTime() : 0;
        ...
        const discountText = maxDiscount ? `Save Up to ${Math.round(Number(maxDiscount))}Rs` : '';
