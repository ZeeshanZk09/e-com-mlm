...
import {
  validateSponsorCode,
  generateUniqueSponsorCode,
  buildHierarchyPath,
} from '@/shared/lib/services/mlm';
import { getMLMSettings, processSignupBonus } from '@/shared/lib/services/mlm';
...
export async function registerUser(formData: RegisterInput): Promise<RegisterResponse> {
  try {
    ...
    const { name, email, password, sponsorCode } = parsedData.data;
    ...
    if (sponsorCode) {
      const sponsorValidation = await validateSponsorCode(sponsorCode);
      if (!sponsorValidation.valid) {
        return { error: sponsorValidation.error || 'Invalid sponsor code' };
      }
    }
    ...
    // Store sponsor code in cookie for use during verification
    if (sponsorCode) {
      cookieStore.set('login_sponsor_code', sponsorCode, { httpOnly: true, maxAge: 300 });
    }
...
export async function verifyEmail(code: string) {
  try {
    ...
    const sponsorCode = cookieStore.get('login_sponsor_code')?.value;
    ...
    // Get MLM settings
    const mlmSettings = await getMLMSettings();

    // Prepare MLM data if sponsor code provided
    let mlmData: {
      sponsorId?: string;
      uplineId?: string;
      hierarchy?: string;
      ...
      isMLMEnabled: mlmSettings.autoEnableMLM,
    };

    let sponsorUser: Awaited<ReturnType<typeof prisma.user.findUnique>> = null;

    if (sponsorCode) {
      const sponsorValidation = await validateSponsorCode(sponsorCode);
      if (sponsorValidation.valid && sponsorValidation.sponsor) {
        sponsorUser = sponsorValidation.sponsor;
        const hierarchy = await buildHierarchyPath(sponsorUser.id);
        mlmData = {
          uplineId: sponsorUser.id,
          hierarchy,
          isMLMEnabled: true,
        };
      }
    ...
    // Generate unique sponsor code for new user
    const newUserSponsorId = await generateUniqueSponsorCode(name);

    // Create user with MLM data
    ...
    // Process signup bonus if user was referred
    if (sponsorUser) {
      try {
        await processSignupBonus(user.id, sponsorUser.id);
      } catch (bonusErr) {
        console.error('Failed to process signup bonus:', bonusErr);
    ...
    cookieStore.delete('login_sponsor_code');
